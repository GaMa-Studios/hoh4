package gama.studios.hoh4core.network;
...
public class Hoh4CoreModVariables {
	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		Hoh4CoreMod.addNetworkMessage(SavedDataSyncMessage.class, SavedDataSyncMessage::buffer, SavedDataSyncMessage::new, SavedDataSyncMessage::handler);
		Hoh4CoreMod.addNetworkMessage(PlayerVariablesSyncMessage.class, PlayerVariablesSyncMessage::buffer, PlayerVariablesSyncMessage::new, PlayerVariablesSyncMessage::handler);
	}
	...
	public static class EventBusVariableHandlers {
		...
		public static void clonePlayer(PlayerEvent.Clone event) {
			...
			clone.adpatationWheelTimer = original.adpatationWheelTimer;
			if (!event.isWasDeath()) {
		...
		public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
			if (!event.getEntity().level().isClientSide()) {
				...
				if (mapdata != null)
					Hoh4CoreMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(0, mapdata));
				if (worlddata != null)
					Hoh4CoreMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(1, worlddata));
			}
		}
	...
	public static class WorldVariables extends SavedData {
		public static final String DATA_NAME = "hoh_4_core_worldvars";
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level level && !level.isClientSide())
				Hoh4CoreMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(level::dimension), new SavedDataSyncMessage(1, this));
		}
	...
	public static class MapVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level && !world.isClientSide())
				Hoh4CoreMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new SavedDataSyncMessage(0, this));
		}
	...
	private static class PlayerVariablesProvider implements ICapabilitySerializable<Tag> {
		...
		public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
			if (event.getObject() instanceof Player && !(event.getObject() instanceof FakePlayer))
				event.addCapability(new ResourceLocation("hoh_4_core", "player_variables"), new PlayerVariablesProvider());
		}
	...
	public static class PlayerVariables {
		...
		public double adpatationWheelTimer = 0;

		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayer serverPlayer)
				Hoh4CoreMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new PlayerVariablesSyncMessage(this));
		}

		public Tag writeNBT() {
			CompoundTag nbt = new CompoundTag();
			nbt.putDouble("status", status);
			...
			nbt.putDouble("adpatationWheelTimer", adpatationWheelTimer);
			return nbt;
		}

		public void readNBT(Tag tag) {
			CompoundTag nbt = (CompoundTag) tag;
			status = nbt.getDouble("status");
			...
			adpatationWheelTimer = nbt.getDouble("adpatationWheelTimer");
		}
	}
		...
		public static void handler(PlayerVariablesSyncMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					...
					variables.adpatationWheelTimer = message.data.adpatationWheelTimer;
				}
			});
			context.setPacketHandled(true);
		}
