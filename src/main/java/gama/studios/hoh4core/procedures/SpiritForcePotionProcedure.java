...
public class SpiritForcePotionProcedure {
			...
			if (Math.random() > 0.7) {
				if (Math.random() > 0.7) {
					if (Math.random() > 0.7) {
						if (Math.random() > 0.7) {
							if (Math.random() > 0.7) {
								if ((entity.getCapability(Hoh4CoreModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new Hoh4CoreModVariables.PlayerVariables())).SpiritForce < (entity
										.getCapability(Hoh4CoreModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new Hoh4CoreModVariables.PlayerVariables())).SpiritForceMax) {
									{
